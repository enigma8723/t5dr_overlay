...
int Memory::GetProcessId(std::string processName) {
    SetLastError(0);
    PROCESSENTRY32 pe32;
    ...
    if( Process32First( hSnapshot, &pe32 ) ) {
        do {
            ...
            for (int i = 0; i < (sizeof(pe32.szExeFile) / sizeof(pe32.szExeFile[0])); i++) {
                currentExeName += pe32.szExeFile[i];
            }

            if(currentExeName.compare(0, processName.length(), processName) == 0)
                break;
...
int Memory::GetPointerAddress(HANDLE processHandle, int64_t startAddress, int offsets[], int offsetCount) {
    if (startAddress == -1)
        return -1;
    int64_t ptr = ReadInt(processHandle, startAddress);
...
int Memory::ReadPointerInt(HANDLE processHandle, int64_t startAddress, int offsets[], int offsetCount) {
    if (startAddress == -1)
        return -1;
